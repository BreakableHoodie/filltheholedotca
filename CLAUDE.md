...
- **Supabase** — Postgres + RLS + storage bucket `pothole-photos`. Application code always reads via short-lived signed URLs; the bucket itself is **still public until `schema_private_photo_bucket.sql` (#32) is applied** — see the photo rules below
