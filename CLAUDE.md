...
- **Supabase** — Postgres + RLS + **private** storage bucket `pothole-photos` (applied 2026-07-27 via `schema_private_photo_bucket.sql` #32; verified `public: false`). All reads go through short-lived signed URLs — see the photo rules below
...
32. `schema_private_photo_bucket.sql` — **security:** flips the `pothole-photos` storage bucket to private so photo objects are no longer reachable at permanent public URLs that outlive an unpublish or reject. All read paths already mint short-lived signed URLs via the service-role client. **Deploy order matters:** ship the signed-URL application code first and confirm photos render, then apply this — running it early breaks every image on the live site until the deploy lands. (#245)
...
- **Signed photo URLs**: the `pothole-photos` bucket is **private** (migration #32, applied 2026-07-27). Direct object access returns 400 at the origin — verified against the live project with both a cache-busted URL and a never-cached object. **Caveat:** flipping the bucket did not purge Cloudflare's edge cache, so a public URL fetched _before_ the flip can still return 200 (`cf-cache-status: HIT`) until its `max-age=3600` expires. Revocation is therefore immediate at the origin but has a ≤1h tail for already-cached objects — assume that window when a takedown is time-sensitive. Never use `getPublicUrl()` — a public object URL is permanent and, once shared or scraped, keeps serving after a photo is unpublished or rejected, outliving the moderation decision. Every read path mints a signed URL server-side via the service-role client (`createSignedUrls`, 1h TTL): `hole/[id]/+page.server.ts`, `admin/photos/+page.server.ts`, `admin/potholes/[id]/+page.server.ts`. Two constraints when changing this: (1) the TTL on `hole/[id]` must stay well above that page's own `Cache-Control` window (`max-age=300` + `stale-while-revalidate=600`), or cached HTML will reference expired URLs; (2) never add an anon SELECT storage policy to "make it work" — signing is done with the service-role key, which bypasses RLS, and granting anon read re-opens the hole. Rejecting a photo also deletes the stored object (all four reject paths).
