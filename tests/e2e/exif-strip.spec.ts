...
test.describe('stripJpegMetadata', () => {
	test('removes APP1/APPn/COM metadata while preserving scan bytes', async () => {
		const app0 = makeSegment(0xe0, [0x4a, 0x46, 0x49, 0x46, 0x00]);
		const app1Exif = makeSegment(0xe1, [0x45, 0x78, 0x69, 0x66, 0x00, 0x00, 0x47, 0x50, 0x53]);
		const com = makeSegment(0xfe, [0x53, 0x65, 0x6e, 0x73, 0x69, 0x74, 0x69, 0x76, 0x65]);
		const dqt = makeSegment(0xdb, [0x00, 0x01, 0x02, 0x03]);
		...
		expect(clean).not.toContain(0xe1);
		expect(clean).not.toContain(0xfe);
		expect(clean).toContain(0xe0);
		expect(clean).toContain(0xdb);
