...
test.describe('Pothole detail page', () => {
	...
	test('shows mark-filled action for the seeded reported pothole', async ({ page }) => {
		await page.goto(fixtureDetailUrl(seededReportedPothole.id));
		await expect(page.getByRole('button', { name: /Mark as filled/i })).toBeVisible();
	});

	test('shows submitted progress state for the seeded pending pothole', async ({ page }) => {
		...
		await expect(page.getByText(/waiting for independent confirmation/i)).toBeVisible();
	...
	test("'I hit this' button is hidden for a pending pothole", async ({ page }) => {
	...
	test("shows the 'Prioritize' button with vote count for a reported pothole", async ({
		...
		await expect(page.getByRole('button', { name: 'Prioritize this pothole' })).toBeVisible();
		await expect(page.getByRole('button', { name: /Prioritize/i })).toContainText('5');
	});

	test("'Prioritize' button is hidden for a pending pothole", async ({ page }) => {
		await page.goto(fixtureDetailUrl(seededPendingPothole.id));
	...
	test("'Prioritize' button is hidden for a filled pothole", async ({ page }) => {
	...
	test('shows the recurring road issue notice when nearbyFilled is populated', async ({
		...
		await expect(page.getByText(/this location may need a permanent repair/i)).toBeVisible();
	...
	test('shows CCC repair card when fixture provides cityRepairRequests', async ({ page }) => {
		...
		await expect(page.getByText(/City repair request on file/i)).toBeVisible();
	...
	test('does not show CCC card for a Waterloo (non-Kitchener) pothole fixture', async ({
		...
		await expect(page.getByText(/City repair request/i)).not.toBeVisible();
	});
