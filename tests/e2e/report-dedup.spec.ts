...
// Pre-computed coordinates against the 25 m merge radius — see
// tests/fixtures/ground-truth.json for the documented ~18 m / ~380 m offsets.
const { basePoint, duplicateReport, distinctReport } = groundTruth.mergeRadius;
...
test.describe('Report API — 25 m merge deduplication', () => {
	...
	test('two reports within 25 m are merged into the same pothole', async ({ request }) => {
		...
		const r2 = await request.post('/api/report', {
			data: {
				lat: duplicateReport.lat,
				lng: duplicateReport.lng,
	...
	test('two reports more than 25 m apart are distinct potholes', async ({ request }) => {
		const r1 = await request.post('/api/report', {
			data: { lat: basePoint.lat, lng: basePoint.lng, description: 'Minor damage' },
		});
		expect(r1.status()).toBe(200);
		const d1 = await r1.json();
		...
		const r2 = await request.post('/api/report', {
			data: {
				lat: distinctReport.lat,
				lng: distinctReport.lng,
				description: 'Minor damage',
			},
		});
		expect(r2.status()).toBe(200);
	...
	test('report outside Waterloo Region geofence is rejected with 422', async ({ request }) => {
	...
	test('report at geofence boundary inside region is accepted', async ({ request }) => {
