...
test.describe("Accessibility — Home (map page)", () => {
  // Suppress the homepage intro card by pre-seeding localStorage at the browser-context level.
  test.use({
    storageState: {
      cookies: [],
      origins: [
...
test.describe("Accessibility — narrow viewport (320px reflow)", () => {
  ...
  test("report page does not scroll horizontally at 320px", async ({
    page,
  }) => {
    await page.goto("/report");
    const scrollWidth = await page.evaluate(
