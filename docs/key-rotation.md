...
| `ADMIN_SESSION_SECRET` | HMAC-SHA-256 for session signing (if applicable) | HMAC-SHA-256 | Invalidates all active admin sessions |
...
1. Generate a new 32-byte hex key:
   ...
   openssl rand -hex 32
   ```
2. Before deploying, re-encrypt all existing TOTP secrets with the new key:
   ```sql
   -- Run this as a one-off script (Node.js) BEFORE deploying the new key:
   -- For each admin_users row with totp_secret IS NOT NULL:
   --   1. decrypt with OLD key
   --   2. re-encrypt with NEW key
   --   3. UPDATE admin_users SET totp_secret = new_ciphertext WHERE id = ...
   ```
   See `src/lib/server/admin-crypto.ts` — use `decryptTotpSecret` with the old key and `encryptTotpSecret` with the new key.
...
## Admin Sessions (ADMIN_SESSION_SECRET / session invalidation)

Sessions are stored as opaque UUIDs in `admin_sessions`. There is no HMAC signing of session IDs — validity is asserted by DB lookup. Invalidating all sessions is therefore a DB operation, not a secret rotation.
...
| Admin sessions | As needed | Admin account compromised |
