...
- [x] **E2E Tests:** Playwright coverage across 26 spec files (17 E2E under `tests/e2e/`, 8 unit-style under `tests/unit/`, 1 a11y under `tests/a11y/`) — report flow, map, navigation, geofence, stats, pothole detail, admin layout/MFA, API schema validation, open data endpoints, security headers, and more.

## Phase 6: Community & Reach ✅ Shipped
...
- [ ] **Accessible non-map data view:** Maps are inherently hard for screen-reader and keyboard users, and the fix is a first-class _alternative_ to the map — not a different map library. (A WebGL/canvas map such as MapLibre would render markers to an opaque `<canvas>`, making it _less_ legible to assistive tech than Leaflet's DOM markers — so Leaflet stays.) Promote today's `sr-only` "Pothole list" into a real, visible, sortable/filterable list or table view that toggles with the map.
