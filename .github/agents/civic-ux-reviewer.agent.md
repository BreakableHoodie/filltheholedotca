...
<button class="min-h-11 min-w-11 touch-manipulation"> Report </button>
...
<input type="text" class="text-base" inputmode="text" autocomplete="street-address" />
...
<!-- Map markers -->
<button aria-label="Pothole reported on {pothole.address}" onclick={() => openDetails(pothole)}>
	📍
</button>
```
...
<section aria-labelledby="councillor-heading">
	...
	<p>{councillor.name}</p>
	<a href="mailto:{councillor.email}"> Email about this pothole </a>
</section>
```
...
{#if potholes.length === 0}
	<div class="text-center py-12">
		...
		<p class="text-zinc-600 mt-2">Be the first to report one in your area</p>
		<a href="/report" class="mt-4 btn-primary"> Report a Pothole </a>
	</div>
{/if}
```
...
{#if error}
	<div role="alert" class="bg-red-50 p-4 rounded">
		...
		<button onclick={retry} class="mt-2 btn-secondary"> Try Again </button>
	</div>
{/if}
```
...
{#if errors.length > 0}
	<div role="alert" aria-live="assertive" class="bg-red-50 p-4">
