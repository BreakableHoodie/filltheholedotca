...
async function getTotpKey(): Promise<CryptoKey> {
  ...
  if (!encryptionKey) throw new Error("env.TOTP_ENCRYPTION_KEY is required");
  const raw = unhex(encryptionKey);
  if (raw.length !== 32)
    throw new Error("env.TOTP_ENCRYPTION_KEY must be 32 bytes (64 hex chars)");
  totpKeyCache = await crypto.subtle.importKey(
    "raw",
    raw,
    { name: "AES-GCM" },
    false,
  ...
  const bits = await crypto.subtle.deriveBits(
    {
      ...
      hash: "SHA-256",
      salt,
      iterations: BACKUP_CODE_ITERATIONS,
    },
    baseKey,
    256,
  );
