...
{#if ids.length > 0}
	<section
		...
		class="bg-stone-100 dark:bg-stone-950 border-t border-stone-200 dark:border-stone-800"
	>
		<div class="max-w-6xl mx-auto px-4 py-8">
			<div class="flex items-center gap-2 mb-5">
				...
				<h2
					...
					class="text-sm font-semibold text-stone-700 dark:text-stone-300"
				>
					My Watchlist
				</h2>
				<span class="text-stone-500 dark:text-stone-400 text-xs tabular-nums ml-1"
					>({ids.length})</span
				>
			</div>
			...
			{#if loading}
				...
				<div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
					{#each [...Array(Math.min(ids.length, PAGE_SIZE)).keys()] as i (i)}
						<div
							class="relative bg-white dark:bg-stone-900 rounded-md overflow-hidden border border-stone-200 dark:border-stone-800 h-[100px] animate-pulse"
						>
							<div
								class="absolute inset-y-0 left-0 w-[3px] bg-stone-300 dark:bg-stone-700"
							></div>
			...
			{:else if fetchError}
				<p class="text-stone-600 dark:text-stone-400 text-sm">
					Couldn't load watchlist status. Try refreshing.
				</p>
			{:else if potholes.length > 0}
				<div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
					{#each visiblePotholes as pothole (pothole.id)}
						...
						<div
							class="relative bg-white dark:bg-stone-900 rounded-md overflow-hidden border border-stone-200 dark:border-stone-800 hover:border-stone-300 dark:hover:border-stone-700 transition-colors"
						>
							<!-- Status accent strip -->
							...
							<div class="pl-5 pr-3 pt-3.5 pb-3.5 flex flex-col gap-2.5">
								...
								<div class="flex items-start justify-between gap-2">
									...
									<button
										...
										aria-label="Remove {pothole.address ||
											'this pothole'} from watchlist"
								...
								<div class="flex items-center justify-between gap-2">
									<span
										class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold {pillColor}"
									>
										<Icon name={info.icon} size={11} />
										{info.label}
									</span>
									<span
										class="text-xs text-stone-500 dark:text-stone-400 tabular-nums shrink-0"
										>{daysLabel(pothole)}</span
									>
								</div>
								...
								<div class="flex items-center justify-between gap-2 pt-0.5">
									<a
										...
										aria-label="View details for {pothole.address ||
											'this pothole'}"
									>
										View details →
									</a>
									{#if pothole.photos_published}
										<span
											class="inline-flex items-center gap-1 text-[11px] text-stone-500 dark:text-stone-400"
										>
