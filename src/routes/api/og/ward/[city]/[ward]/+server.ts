...
export const GET: RequestHandler = async ({ params }) => {
	...
	const councillor = COUNCILLORS.find((c) => c.city === (city as City) && c.ward === wardNum);
	...
	const wardPotholes = (data ?? []).filter((p) =>
		inWardFeature(p.lng, p.lat, wardFeature.geometry),
	);
	...
	const avgDays =
			...
			: done.reduce(
					(s, p) =>
						s + new Date(p.filled_at!).getTime() - new Date(p.created_at).getTime(),
					0,
				) /
				done.length /
				86_400_000;
	...
	const svg = await satori(
		el(
			...
			el(
				'div',
				{ style: { display: 'flex', alignItems: 'center', gap: 40, marginBottom: 40 } },
				el(
					'div',
					{ style: { fontSize: 120, fontWeight: 700, color: gradeHex, lineHeight: 1 } },
					grade,
				),
				el(
					'div',
					...
					el(
						'div',
						{ style: { fontSize: 32, color: '#e4e4e7', fontWeight: 700 } },
						fillLabel,
					),
					el(
						'div',
						{ style: { fontSize: 28, color: '#71717a' } },
			...
			el(
				'div',
				{ style: { fontSize: 28, color: '#38bdf8', letterSpacing: '0.04em' } },
				'fillthehole.ca',
			),
		),
		{
