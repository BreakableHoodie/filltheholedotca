<script lang="ts">
	...
	let open = $derived(
		wardPotholes.filter((p) => p.status === 'reported' || p.status === 'expired').length,
	);
			...
			try {
				...
				wardNotifState =
					Notification.permission === 'denied'
						? 'denied'
						: localStorage.getItem(`ward-notify:${key}`) === '1'
							? 'subscribed'
							: 'unsubscribed';
			} catch {
	...
	let cityLabel = $derived(city.charAt(0).toUpperCase() + city.slice(1));
</script>
...
<svelte:head>
	...
	<meta
		name="description"
		content="Ward {ward} {cityLabel} pothole accountability: grade {grade.grade}, {fillRate !==
		null
			? fillRate.toFixed(0) + '% fill rate'
			: 'no data yet'}. {open} open potholes."
	/>
	<meta
		property="og:title"
		content="{councillor.name} · Ward {ward} Accountability — FillTheHole.ca"
	/>
	<meta
...
<div class="max-w-2xl mx-auto px-4 py-8 space-y-6">
	...
	<div class="bg-stone-900 border border-stone-800 rounded-md p-5">
		<div class="flex items-start gap-4">
			<div
				class="shrink-0 w-12 h-12 rounded-full bg-amber-500 flex items-center justify-center text-white font-bold text-xl select-none"
			>
		...
		{#if wardNotifState !== 'unsupported'}
			<button
				type="button"
				onclick={wardNotifState === 'subscribed'
					? unsubscribeWardNotification
					: subscribeWardNotification}
				disabled={wardNotifState === 'pending' || wardNotifState === 'denied'}
				class="mt-4 w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-md text-sm font-semibold transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:ring-offset-2 focus-visible:ring-offset-stone-900 disabled:opacity-60 {wardNotifState ===
				'subscribed'
	...
	<div class="grid grid-cols-3 gap-3">
		...
		<div class="bg-stone-900 border border-stone-800 rounded-md p-4 text-center">
			<p class="text-2xl font-bold {open > 0 ? 'text-orange-400' : 'text-stone-500'}">
				{open}
			</p>
	...
	<div class="bg-stone-900 border border-stone-800 rounded-md p-5">
		<div class="flex items-center justify-between mb-3">
			...
			<div class="flex items-center gap-3 text-xs text-stone-500">
				<span class="flex items-center gap-1"
					><span class="w-2.5 h-2.5 rounded-sm bg-orange-500/70 inline-block"
					></span>Reported</span
				>
				<span class="flex items-center gap-1"
					><span class="w-2.5 h-2.5 rounded-sm bg-green-500/70 inline-block"
					></span>Filled</span
				>
			</div>
		</div>
		<div
			...
			role="img"
			aria-label="Bar chart of monthly pothole reports and fills for this ward over the last 12 months"
		>
				...
				<div class="flex-1 flex flex-col items-stretch gap-px">
					<div class="flex items-end gap-px flex-1">
						<div
							...
							style="height:{Math.round((m.reported / monthlyMax) * 72)}px"
							title="{m.reported} reported in {m.label}"
						></div>
						<div
							class="flex-1 bg-green-500/70 rounded-t-sm"
							style="height:{Math.round((m.filled / monthlyMax) * 72)}px"
				...
				{#each openPotholes as p (p.id)}
					<li>
						...
						>
							...
							<span class="shrink-0 text-stone-600 tabular-nums text-xs"
								>{p.days}d</span
							>
						</a>
					</li>
