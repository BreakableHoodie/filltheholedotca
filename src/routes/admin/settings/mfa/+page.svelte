<script lang="ts">
	...
	const pendingSetupError = $derived(
...
<div class="p-6 max-w-2xl">
	...
	{#if form?.confirmed && form.backupCodes}
		<div class="bg-emerald-500/10 border border-emerald-500/30 rounded-lg p-5 mb-5">
			...
			<p class="text-stone-300 text-sm mb-4">
				Save these backup codes somewhere safe. Each can only be used once if you lose
				access to your authenticator.
			</p>
			<div class="grid grid-cols-2 gap-2">
				{#each form.backupCodes as code (code)}
					<code
						class="bg-stone-900 border border-stone-700 rounded px-3 py-1.5 text-sm font-mono text-stone-200 text-center"
					>
						{formatCode(code)}
					</code>
				{/each}
			...
			<p class="text-stone-500 text-xs mt-3">
				These codes are shown once and cannot be retrieved later.
			</p>
		</div>
	...
	{:else if form?.newBackupCodes}
		<div class="bg-amber-500/10 border border-amber-500/30 rounded-lg p-5 mb-5">
			...
			<p class="text-stone-300 text-sm mb-4">
				Your old codes are now invalid. Save these new ones.
			</p>
			<div class="grid grid-cols-2 gap-2">
	...
	{:else if form?.disabled}
		<div
			class="mb-5 px-4 py-3 bg-stone-800 border border-stone-700 rounded-lg text-stone-300 text-sm"
		>
			...
			<div>
				...
				<div class="space-y-2">
					<div>
						...
						<code
							class="block bg-stone-800 border border-stone-700 rounded px-3 py-2 text-sm font-mono text-stone-200 break-all select-all"
						>
							{form.displaySecret ?? ''}
						</code>
					</div>
					{#if form.totpUri}
						<details class="group">
							<summary
								class="text-xs text-stone-500 cursor-pointer hover:text-stone-300 transition-colors"
							>
								Show TOTP URI
							</summary>
							<code
								class="block mt-1 bg-stone-800 border border-stone-700 rounded px-3 py-2 text-xs font-mono text-stone-400 break-all select-all"
							>
								{form.totpUri}
							</code>
						</details>
			...
			<form method="post" action="?/confirmMfa" use:enhance class="space-y-3">
				...
				<div class="flex items-center gap-3">
					...
					<a
						...
						class="text-sm text-stone-500 hover:text-stone-300 transition-colors"
					>
						Cancel
					</a>
				</div>
	...
	{:else if !data.totpEnabled && !form?.confirmed}
		<div class="bg-stone-900 border border-stone-800 rounded-lg p-5">
			<div class="flex items-start gap-3 mb-4">
				<div
					class="w-8 h-8 rounded-full bg-amber-500/10 flex items-center justify-center flex-shrink-0 mt-0.5"
				>
					<svg
	...
	{:else if data.totpEnabled || form?.confirmed}
		<div class="space-y-4">
			...
			<div class="bg-stone-900 border border-stone-800 rounded-lg p-5">
				...
				<details class="group">
					<summary
						class="text-xs text-stone-500 cursor-pointer hover:text-red-400 transition-colors"
					>
						Disable MFA…
					</summary>
					<form method="post" action="?/disableMfa" use:enhance class="mt-3 space-y-2">
						...
						<div class="flex items-center gap-2">
							...
							<button
								...
								onclick={(e) => {
									if (!confirm('Disable MFA? Your account will be less secure.'))
										e.preventDefault();
								}}
							>
								Disable
			...
			<div class="bg-stone-900 border border-stone-800 rounded-lg p-5">
				...
				<p class="text-stone-500 text-sm mb-3">
					Generate a fresh set of single-use backup codes. Your old codes will be
					invalidated.
				</p>
