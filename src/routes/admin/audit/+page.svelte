...
<div class="p-6">
	...
	<form method="get" class="flex flex-wrap items-end gap-3 mb-5">
		...
		<div>
			<label for="filter-resource" class="block text-xs text-stone-500 mb-1"
				>Resource type</label
			>
			<select
				id="filter-resource"
		...
		<div>
			...
			<div class="flex gap-2">
				...
				{#if data.filterUser}<input
						type="hidden"
						name="user"
						value={data.filterUser}
					/>{/if}
				{#if data.filterResourceType}<input
						type="hidden"
						name="resource_type"
						value={data.filterResourceType}
					/>{/if}
				<button
					type="submit"
			...
			<table class="w-full text-sm min-w-[640px]">
				<thead>
					<tr class="border-b border-stone-800 text-left">
						<th
							class="px-4 py-3 text-xs font-medium text-stone-500 uppercase tracking-wide w-44"
							>Time</th
						>
						<th
							class="px-4 py-3 text-xs font-medium text-stone-500 uppercase tracking-wide"
							>Actor</th
						>
						<th
							class="px-4 py-3 text-xs font-medium text-stone-500 uppercase tracking-wide"
						...
						>
						<th
							class="px-4 py-3 text-xs font-medium text-stone-500 uppercase tracking-wide w-32"
							>IP</th
						>
					</tr>
				</thead>
				<tbody class="divide-y divide-stone-800">
					{#each data.entries as entry (entry.id)}
						<tr class="hover:bg-stone-800/40 transition-colors">
							...
							<!-- Actor -->
							<td class="px-4 py-3">
								<span class="text-stone-200 text-xs font-medium"
									>{actorLabel(entry)}</span
								>
								{#if actorEmail(entry)}
									<span class="block text-stone-600 text-xs"
										>{actorEmail(entry)}</span
									>
								{/if}
							</td>
							...
							<td class="px-4 py-3">
								{#if entry.resource_type}
									<span class="text-stone-400 text-xs capitalize"
									...
									{#if entry.resource_id}
										...
										{:else}
											<span
												class="block text-stone-600 text-xs font-mono mt-0.5"
											>
												{entry.resource_id.slice(0, 8)}…
											</span>
										{/if}
							...
							<td class="px-4 py-3 max-w-xs">
								{#if entry.details}
									<span
										class="text-stone-600 text-xs font-mono break-all"
	...
	{#if data.totalPages > 1}
		<div class="flex items-center justify-between mt-4">
			<div>
				...
				{:else}
					<span
						class="px-3 py-1.5 text-sm text-stone-700 border border-stone-800 rounded cursor-not-allowed"
					>
						← Previous
					</span>
				{/if}
